...
namespace Homework.Control_Work.CW2
{
    public class Linq
    {
        ...
        public void Run()
        {
            ...
            void Task_5()
            {
                foreach (var discount in Task_4())
                    Console.WriteLine($"{string.Join(" + ", discount.Item1.GroupBy(x => x.Id).Select(x => $"{x.Count()} {x.First().Name}"))} {GetCheckSum(discount.Item1) * (100 - discount.Item2) / 100} / {GetCheckSum(discount.Item1)}  ");
            }

            decimal GetCheckSum(List<Product> check) => check.Select(x => prices.Find(price => price.ProductId == x.Id).Sum).Sum();
        }
    }
}
